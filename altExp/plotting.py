...
#                                      predStd[d, v_sfs, v_cons[c]], fmt='p', clip_on=False);
        ...
        #dispAx[d][c_plt_ind, 0].plot(sfs_plot, hf.flexible_Gauss(curr_mod_params, sfs_plot), clip_on=False)
...
for d in range(nDisps):
    ...
    for c in reversed(range(n_v_cons)):
        ...
        # plot descriptive model fit
        curr_mod_params = descrFits[d, v_cons[c], :];
        sfMixAx[c_plt_ind, d].plot(sfs_plot, hf.flexible_Gauss(curr_mod_params, sfs_plot), clip_on=False)
