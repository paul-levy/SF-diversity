...
%   program used for the sfMixLGN experiment. The input unitLabels is either
...
addpath(genpath('/Users/paulgerald/work/sfDiversity/sfDiv-OriModel/sfDiv-python/LGN/analysis/uvnlExpo/'));
% addpath(genpath('/u/vnl/matlab/ExpoMatlab/'));

supportedPrograms = {'sfMixLGN'};
supportedTypes    = {'sfm'};
...
for iU = 1:nUnits
    ...
    for iF = FileNDX
        ...
        if any(matchesSelected) && any(matchesSupported)
            ...
            [s] = AnalyzeExperiment(loadPath, expType, xmlList(iF).name, getFullWave);
            expStruct.exp = s;
...
%%
%% AnalyzeExperiment
function [s] = AnalyzeExperiment(loadPath, expType, fileName, getFullWave)
