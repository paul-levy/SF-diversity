...
% sfMixLoad       Sorting of sfMix xml-files.
%
%   sfMixLoad(unitLabels) loads the xml-files associated with the Expo
%   program used for the sfMix experiment. The input unitLabels is either
...
%
%   sfMixLoad(unitLabels, 'loadPath', path) specifies the directory from
%   which to load xml files. Default is the current directory.
%
%   sfMixLoad(unitLabels, 'savePath', path) specifies the directory to
%   which resulting structures will be saved (one file per unit). Default
%   is to match loadPath.
%
%   sfMixLoad(unitLabels, 'processRepeats', 1) enables processing multiple
...
nCondSfMix  = (DIS.matrix.Dimensions{1}.Size-1)*(DIS.matrix.Dimensions{2}.Size-2)*DIS.matrix.Dimensions{3}.Size;
nCondOri    = 16;
nCondCon    = 10;
nCondBlank  = 1;
nConditions = nCondSfMix + nCondOri + nCondCon + nCondBlank;
...
s.ori           = oriVals;
s.oriRateMean   = oriRateMean;
s.oriRateVar    = oriRateVar;
s.oriModRatio   = oriModRatioGeoMean;
s.con           = conVals;
