function [M] = retinaSimpleResp(varargin)

% retinaSimpleResp    Computes response of simple cell for sfmix experiment
%
% retinaSimpleResp(varargin) returns a simple cell response for the
% mixture stimuli used in sfMix. The cell's receptive field is modeled as 
% arising from a cascaded process. First, a retina-like stage that 
% processes the stimulus in two parallel channels (center-surround 
% receptive fields followed by a rectifying nonlinearity). Second, a linear
% filtering process that weights both channels, sharpens spatial frequency
% tuning and creates orientation selectivity (the filter is an n-th
...
% Get eccentricity and LGN weights
ecc    = channel.ecc;
wMagno = channel.wMagno;

% Store some results in M (M is NOT magno)
M          = struct;
M.pref.or  = prefOr;
M.pref.sf  = prefSf;
M.pref.tf  = prefTf;
...
for p = 1:nTrials
    ...
    % 0. LGN stage: Rescale the stimulus contrast according to the selectivity of the pre-cortical channels
    freqsUnitResp  = logspace(-2, 2, 128);                                 % Used to normalize responses to unit range
    [magno, parvo] = make_LGN(ecc, freqsUnitResp);
        
    stimEnergy = sqrt(sum(stimCo.^2));
    
    selSfMagno = magno.sf(stimSf);
    respMagno  = sum(selSfMagno.*stimCo);
    scaleMagno = (respMagno/ (stimEnergy + magno.c50)); %magno.crf(respMagno);     
    conMagno   = (selSfMagno.*stimCo) * scaleMagno;
    
    selSfParvo = parvo.sf(stimSf);
    respParvo  = sum(selSfParvo.*stimCo);
    scaleParvo = (respParvo / (stimEnergy + parvo.c50)); %parvo.crf(respParvo); 
    conParvo   = (selSfParvo.*stimCo) * scaleParvo;
    
    stimCo = wMagno*conMagno + (1-wMagno)*conParvo;
