function M = SFMNormResp(unitName, varargin)
    ...
    for p = 1:nTrials
        ...
        for c = 1:9                                                    % there are up to nine stimulus components
            ...
            if stimCo(c) ~= 0 %if (selSi ~= 0 && stimCo(c) ~= 0)
                ...
                rComplex_static{c} = stimCo(c)*exp(1i*P*[omegaX(c) omegaY(c) omegaT(c)]');
        ...
        for iF = 1:nSf
            ...
            for c = 1:9                                                    % there are up to nine stimulus components
                ...
                if (selSi ~= 0 && stimCo(c) ~= 0)
                    computeSum = 1;
                    % now we're mostly repeating a simple multiply rather
                    % than exp...
