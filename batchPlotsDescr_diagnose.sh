...
if [ "$EXP_DIR" = "V1/" ]; then
  for run in {1..30}
  do 
    python3.6 plot_descr_diagnose.py $run V1/ $DOG_MOD $LOSS_TYPE $JOINT -1 1 1 $HPC &
  done
  wait
  for run in {31..60}
  do 
...
if [ "$EXP_DIR" = "LGN/" ]; then
  ...
  do 
    # mov RVC
    #python3.6 plot_descr_diagnose.py $run LGN/ $DOG_MOD $LOSS_TYPE $JOINT 1 0 1 $HPC 0 &
    # N-R RVC 
    #python3.6 plot_descr_diagnose.py $run LGN/ $DOG_MOD $LOSS_TYPE $JOINT 1 1 1 $HPC 0 &
...
if [ "$EXP_DIR" = "V1_orig/" ]; then
  for run in {1..20}
  do 
    python3.6 plot_descr_diagnose.py $run V1_orig/ $DOG_MOD $LOSS_TYPE $JOINT 0 1 1 $HPC &
  done
  wait
  for run in {21..40}
  do 
...
if [ "$EXP_DIR" = "altExp/" ]; then
  for run in {1..8}
  do 
    python3.6 plot_descr_diagnose.py $run altExp/ $DOG_MOD $LOSS_TYPE $JOINT 0 1 1 $HPC &
  done
fi
if [ "$EXP_DIR" = "V1_BB/" ]; then
  for run in {1..23}
  do 
    python3.6 plot_descr_diagnose_sfBB.py $run V1_BB/ $DOG_MOD $LOSS_TYPE $JOINT 1 1 $HPC 0 0 &
  done
  wait
  for run in {24..47}
  do 
