...
'''
Based on the optimization procedure in mod_resp_trackNLL, we can use this function/script to evaluate the quality of the model fits.
Per conversation with Tim Oleskiw, arising from a practice talk I gave on 10.03.18, the following procedure is meant to be an unbiased procedure
for comparing models that might differ in parameterization/complexity. The analysis description as follows is specifically for a dataset like my own,
and shouldn't be taken as a general procedure!

For each cell, do the following:

  For each stimulus condition (recall we have 5 dispersions x 2 contrasts x 11 spatial frequency centers = 110 conditions), we fit the full model while holding
  out the data from that particular condition. We can then derive two measures of fit quality: one is the overall loss (per trial or condition) and the second,
  perhaps more informative, is the loss (or error) on the prediction for the held out condition based on the optimized parameters.
...
if __name__ == '__main__':
    ...
    if fits_curr is None:
      ...
      if collapseSF == 1:
        ...
        holdoutNLLs = numpy.nan * numpy.empty((nDisps, nCons));
        allParams = numpy.array(numpy.nan * numpy.empty((nDisps, nCons)), dtype='O');
      else:
    ...
    if collapseSF:
      conds_as_tuple = zip(conds_rem[0], conds_rem[1]);
    else:
    ...
    for i in conds_as_tuple:
      ...
      if collapseSF:
        fits_curr['NLL'][d, c] = NLL;
        fits_curr['allParams'][d, c] = params;
        fits_curr['holdoutNLL'][d, c] = holdoutNLL;
      else:
