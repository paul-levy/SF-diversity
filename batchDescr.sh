...
#   9 - loss type (for DoG fit); 
#       1 - lsq
#       2 - sqrt
#       3 - poiss [was previously default]
#       4 - Sach sum{[(exp-obs)^2]/[k+sigma^2]} where
#           k := 0.01*max(obs); sigma := measured variance of the response
#   [10 - phase direction (pos or neg)]; default is pos (check...)
#   [11 - regularization for gain term (>0 means penalize for high gain)] default is 0

### GUIDE (as of 19.09.16)
# V1/ - use dataList_glx.npy, 16 cells
...
for run in {1..16}
do
  ## LGN - phase adjustment and F1 rvc
